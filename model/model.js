...
var index = {
		...
		queryNextEn:'SELECT t.id,t.code,t.nameEn name FROM news t WHERE t.`createDate`<(SELECT createDate FROM news WHERE code= ? ) and t.type= ? ORDER BY t.createDate LIMIT 1',
...
var news = {
		...
		queryNewsListEnAll:'SELECT t.id,t.code,t.nameEn name,t.createBy,DATE_FORMAT(t.createDate,"%Y/%c/%d") createDate FROM news t ORDER BY t.createDate DESC LIMIT ? , ? ',
		queryNewsListZhAll:'SELECT t.id,t.code,t.nameCh name,t.createBy,DATE_FORMAT(t.createDate,"%Y/%c/%d") createDate FROM news t ORDER BY t.createDate DESC LIMIT ? , ? ',
